...
export default function Update() {
    ...
    function handleOnChange(event: ChangeEvent<HTMLInputElement>) {
        ...
        const value = Array.from(event.target.value.replace(/[^a-zA-Z0-9]/g, ''))
        const splitValue = value.reduce((acc: string[], curr, index) => {
            if (index % 5 === 0) {
                return [...acc, value.slice(index, index + 5).join('')];
            }
            return acc;
        }, [] as string[])
        if (value.length <= 15) setSerialNumber(splitValue.join('-'));
    }
    ...
    async function onSubmitUpdate(event: FormEvent<HTMLFormElement>) {
        ...
        formData.set("serialNumber", serialNumber.replace(/[^a-zA-Z0-9]/g, ''))
